/*
  Controle de 2 LEDs + 1 Servo Motor com velocidade definida por software
  ...
  - Velocidade do Servo: definida fixamente pela variável "tempo" no código,
    sem necessidade de potenciômetro externo.
      tempo = 1   -> movimento o mais rápido possível
      tempo = 100 -> movimento o mais lento possível
  - Alto-falante de aviso (pino 10): sempre que um botão válido é
    apertado, o sistema entra em uma "fase de aviso" de 3 segundos,
    durante a qual o alto-falante emite um bipe intermitente (usando a
    função tone(), que gera um tom audível de verdade - o alto-falante
    usado aqui é PASSIVO, ou seja, precisa de um sinal de áudio, não
    apenas de HIGH/LOW). Só DEPOIS desses 3 segundos o LED realmente
    troca e o servo começa a se mover. O bipe continua intermitente
    durante todo o movimento do servo, e só para completamente quando o
    servo chega ao destino (0 ou 180 graus).
  ...
  - Alto-falante (passivo, 100 ohms / 0,25W): um terminal -> pino 10
    (idealmente com um resistor de ~100 ohms em série, para limitar a
    corrente), outro terminal -> GND
    (NÃO usar o alto-falante de celular das fotos 1/2: a impedância dele
    é baixa demais e puxaria corrente excessiva do pino do Arduino;
    o alto-falante de 100 ohms / 0,25W é o adequado para ligar direto
    num pino digital)
*/
...
const int pinoBuzina = 10;  // Alto-falante de aviso (passivo, 100 ohms / 0,25W)
...
// Ajuste este valor diretamente no código para mudar a velocidade do motor:
//   Valores menores (ex: 1 a 5)   -> movimento mais RÁPIDO
//   Valores maiores (ex: 30 a 100) -> movimento mais LENTO
int tempo = 15;
...
// Frequência (em Hz) do tom emitido pelo alto-falante durante o bipe.
// Como o alto-falante é passivo, precisamos gerar um tom de verdade
// com tone() - um simples HIGH/LOW não produziria som contínuo nele.
// Ajuste esse valor para deixar o som mais grave (número menor) ou
// mais agudo (número maior).
int frequenciaBuzina = 2000;
...
// ajustada via software para definir o intervalo entre cada grau do movimento.
...
void iniciarTroca(int ledAlvo) {
  ...
  tone(pinoBuzina, frequenciaBuzina); // começa a emitir o tom no alto-falante
}
  ...
  if (fase == AVISANDO || fase == MOVENDO) {
    if (millis() - ultimoBeepToggle >= intervaloBeep) {
      ...
      if (buzinaLigadaAgora) {
        tone(pinoBuzina, frequenciaBuzina); // liga o tom
      } else {
        noTone(pinoBuzina); // silêncio (alto-falante passivo precisa disso)
      }

      ultimoBeepToggle = millis();
    }
  } else {
    // Fase PARADO: garante que o alto-falante fique em silêncio
    if (buzinaLigadaAgora) {
...
void setup() {
  ...
  // Define o pino do alto-falante como saída, já começando em silêncio
